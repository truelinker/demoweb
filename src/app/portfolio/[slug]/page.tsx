...
export async function generateStaticParams() {
  ...
  // Filter for only portfolio posts
  const portfolioSlugs = slugs.filter(slug => {
    const post = getMDXPostBySlug(slug);
    return post?.category === 'portfolio';
  });
  
  return portfolioSlugs.map((slug: string) => ({
    slug,
  }));
}
...
export async function generateMetadata(props: {
  ...
  return {
    title: `${post.title} | Portfolio | Demo Web Service`,
    description: post.excerpt,
  };
}

// Make sure the component is async to handle params properly
export default async function PortfolioPostPage(props: {
  params: Promise<{ slug: string }>
}) {
  // IMPORTANT: Await the params before using them
  const params = await props.params;
  ...
  if (!post || post.category !== 'portfolio') {
    notFound();
  }
    ...
    <div className="portfolio-post-container">
      ...
      <div className="container mx-auto px-4 py-8">
        <article className="max-w-4xl mx-auto">
          ...
          <div className="prose prose-lg dark:prose-invert max-w-none portfolio-content
