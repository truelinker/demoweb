...
```
  ...
  * Simultaneous TX/RX        * Takes turns on shared line
  * Two separate lines        * One shared line
```
*Figure 1: Comparison of full-duplex (left) and half-duplex (right) communication modes. In full-duplex, both devices can transmit and receive simultaneously on separate lines. In half-duplex, devices must take turns using the single shared line.*
...
- **Packet-based communication**: All messages are encapsulated in packets with headers and checksums
...
struct SIOPacket {
    ...
    uint8_t packetType;    // Command, Response, or Error
    uint8_t length;        // Length of payload
    uint8_t payload[64];   // Variable length data
    uint16_t checksum;     // CRC-16 of all preceding bytes
    uint8_t endMarker;     // Always 0x55
};
```
...
- **Priority mechanism**: Assigning different priorities to different types of messages
...
```
  ...
  +------------------+    Success
           ...
           |
  +--------v-------+   +----------------+   +--------------+
  | Timeout Handler|   |Resynchronization|   |Fallback Mode |
  +----------------+   +----------------+   +--------------+
           |                   |                   |
           |                   |                   |
           v                   v                   v
  ...
  +------------------+  |
                 ...
                 +-------------+
```
*Figure 4: State machine diagram showing error detection and recovery strategies implemented in the bootloader*
